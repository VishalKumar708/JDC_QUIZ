...
class QuizEnrollmentFilter(django_filters.FilterSet):
    ...
    status = django_filters.ChoiceFilter(
        choices=QuizEnrollment.STATUS_CHOICES,
        field_name='status',
        lookup_expr='exact',
        label='Display the user status indicating those who are currently enrolled, playing (incomplete), or have completed the quiz.')
